...
description: Screenplay format, page craft, the writing process and adaptation (剧本格式/写作流程/改编) — merged from Wendy Henson (spec format rules), Richard Walter (page craft, revision), Neill Hicks (selling vs shooting script, draft chain), Syd Field (master shot, card method, adaptation as original work), Robert McKee (step outline → treatment → screenplay), Julian Hoxter (format elements, adaptation four questions), Diamond & Weissman (one-page nine-step) and Eric Bork — plus an exact typographic grid, element conventions (V.O./O.S., MORE/CONT'D, dual dialogue), a machine-writable output contract (Fountain, with forced markers for Chinese), and Asian industry formats (Chinese 场号制 影视剧本格式, Japanese 柱・ト書き・セリフ / 決定稿). Use when formatting a spec script, choosing the output file format, deciding what goes in action lines, planning drafts and outlines, cutting pages, adapting a novel, play or true story, writing a Chinese TV/film or Japanese-style script page, or preparing a submission-ready draft.
---
...
1. "N 页"先读成"**N 分钟的戏**"。
2. 中文每分钟字数：**对白为主取 370–400 字／分钟；动作与反应多的（悬疑、动作、单机位喜剧）取 270–330 字／分钟**。**最低一档：以成段无声动作为主的稿件（默片式段落、长镜头行动戏、小津式事件画外）取 230–260 字／分钟**——这类本子上 270 仍偏高，会把实拍时长系统性低估。取这一档时**必须在交付件里写明折算依据**（哪几场按无声动作计、字数 ÷ 每分钟字数 ＝ 折算页）。
3. 折算页 ＝ 非空白字数 ÷ 每分钟字数。
